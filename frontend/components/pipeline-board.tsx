...
const STATUS_COLUMNS = [
    ...
    { id: "offer_rejected", label: "Offer Declined" },
...
const STATE_ACTIONS: Record<string, { action: string; label: string; variant: 'primary' | 'destructive' | 'secondary' | 'success' }[]> = {
    ...
    interview_completed: [
        { action: "hire", label: "Hire & Send Offer", variant: "success" },
    ...
    // offer_sent: candidate-driven via email — no HR actions on pipeline board
    // offer_accepted: system-driven only
    // offer_rejected / onboarded / rejected: terminal
}
...
export function PipelineBoard({ jobId }: { jobId?: string }) {
    ...
    const handleTransition = async (applicationId: number, action: string, notes?: string) => {
        if (action === 'hire') {
            // Intercept hire — open joining date popup instead of direct FSM call
            const app = applications.find(a => a.id === applicationId)
            const today = new Date()
            today.setDate(today.getDate() + 1)
            setJoiningDate(today.toISOString().split('T')[0])
            setHireDialogApp({ id: applicationId, name: app?.candidate?.full_name ?? 'Candidate' })
            return
        }
    ...
    const isRejectAllowed = (status: string) => {
        return ["applied", "screened", "interview_scheduled", "review_later", "physical_interview"].includes(status)
    }
        ...
        {hireDialogApp && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                <div className="bg-card border border-border rounded-2xl shadow-2xl p-6 w-full max-w-sm mx-4 animate-in fade-in zoom-in-95 duration-200">
                    <h2 className="text-lg font-bold text-foreground mb-1">Hire & Send Offer</h2>
                    <p className="text-sm text-muted-foreground mb-5">
                        Hiring <span className="font-semibold text-foreground">{hireDialogApp.name}</span>. Select a joining date — the offer letter will be generated and sent immediately.
                    </p>
                    <div className="mb-5">
                    ...
                    <div className="flex gap-2 justify-end">
                        ...
                        <button
                            ...
                            className="px-4 py-2 rounded-lg text-sm font-bold bg-emerald-600 hover:bg-emerald-700 text-white transition-colors disabled:opacity-50 shadow-sm"
                        >
                            {hireLoading ? 'Sending...' : 'Confirm & Send Offer'}
                        </button>
                    </div>
                </div>
            </div>
