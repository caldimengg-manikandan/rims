...
State Machine is defined by STATES_README.md. Do not add states here that
are not defined in that document.
...
Design principles:
  ...
  4. State history is logged to AuditLog
...
# ─────────────────────────────────────────────────────────────────────────────
# 1. Terminal States & Core Definitions
# ─────────────────────────────────────────────────────────────────────────────

# Terminal states — no HR transitions out of these
...
# OFFER_ACCEPTED is semi-terminal: only system_onboard can move it forward
SEMI_TERMINAL_STATES = frozenset({
...
_TRANSITION_TABLE: Dict[Tuple[CandidateState, TransitionAction], CandidateState] = {
    ...
    # 3. interview_scheduled → interview_completed or rejected
    ...
    # 4. interview_completed → offer_sent / physical_interview / review_later
    #    Hire triggers the offer flow directly — no persistent 'hired' state.
    (CandidateState.INTERVIEW_COMPLETED, TransitionAction.HIRE): CandidateState.OFFER_SENT,
    ...
    # 6. physical_interview → offer_sent / rejected
    #    Hire triggers the offer flow directly — no persistent 'hired' state.
    (CandidateState.PHYSICAL_INTERVIEW, TransitionAction.HIRE): CandidateState.OFFER_SENT,
    (CandidateState.PHYSICAL_INTERVIEW, TransitionAction.REJECT): CandidateState.REJECTED,

    # 7. offer_sent → offer_accepted / offer_rejected  (candidate-driven via email)
    (CandidateState.OFFER_SENT, TransitionAction.ACCEPT_OFFER): CandidateState.OFFER_ACCEPTED,
    (CandidateState.OFFER_SENT, TransitionAction.DECLINE_OFFER): CandidateState.OFFER_REJECTED,
    # Legacy: REJECT action on offer_sent means candidate declined
    (CandidateState.OFFER_SENT, TransitionAction.REJECT): CandidateState.OFFER_REJECTED,

    # 8. offer_accepted → onboarded  (system-driven)
    (CandidateState.OFFER_ACCEPTED, TransitionAction.SYSTEM_ONBOARD): CandidateState.ONBOARDED,
}

# Email mapping: (action, target_state) → email_type identifier
EMAIL_TRIGGERS: Dict[Tuple[TransitionAction, CandidateState], str] = {
    ...
    (TransitionAction.HIRE, CandidateState.OFFER_SENT): "hired",
    (TransitionAction.DECLINE_OFFER, CandidateState.OFFER_REJECTED): "offer_rejected",
}
...
# ─────────────────────────────────────────────────────────────────────────────
# 4. Friendly Labels for Non-Technical Users
# ─────────────────────────────────────────────────────────────────────────────
...
class CandidateStateMachine:
    ...
    def get_allowed_actions(self, application: Application) -> List[str]:
        ...
        if current in SEMI_TERMINAL_STATES:
            # Only system actions allowed
            return [
                action.value for (state, action), _ in _TRANSITION_TABLE.items()
                if state == current
            ]
    ...
    def validate_transition(
        ...
        # Semi-terminal: only system actions
        if current in SEMI_TERMINAL_STATES and not action.value.startswith("system_"):
            raise InvalidTransitionError(
                current.value, action.value,
                f"Only system actions are allowed from state '{STATE_DISPLAY_NAMES.get(current.value, current.value)}'"
            )
    ...
    def transition(
        ...
        # 3. Preconditions (including notes if required)
        ...
        # 6. Handle Automated Side Effects
    ...
    def _trigger_interview_report(self, application: Application, background_tasks):
        """Logic to trigger AI report generation with safety checks."""
    ...
    def _check_preconditions(
        ...
        # (unless coming from physical_interview where there may be no digital interview)
...
# ─────────────────────────────────────────────────────────────────────────────
# 7. UI Button Mapping Helpers
# ─────────────────────────────────────────────────────────────────────────────

def get_ui_buttons_for_state(state: str) -> List[Dict[str, str]]:
    """
    Return the list of UI action buttons for each pipeline state (spec-compliant).

    Application Pipeline:
      applied             → Mark as Screened, Reject
      screened            → Approve for Interview, Reject
      interview_scheduled → (waiting state — no transition buttons)
    ...
    Onboarding Pipeline:
      offer_sent          → (candidate-driven via email — no HR buttons)
      offer_accepted      → (system-driven only)
